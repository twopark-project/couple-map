...
class _HomeScreenState extends ConsumerState<HomeScreen>
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      if (authState is AuthSuccess) {
        ...
        // authProvider에 상태가 없으면 저장된 토큰에서 읽기
      ...
      final results = await Future.wait([
        _mypageRepository.getUserInfo(accessToken),
        _homeRepository.getMapList(accessToken),
        _friendRepository.getFriendList(accessToken),
        _friendRepository.getPendingFriendList(accessToken),
      ]);
  ...
  Future<void> _logout() async {
    try {
      final authState = ref.read(authProvider);
      if (authState is AuthSuccess) {
        await _authRepository.logout(authState.token.accessToken);
      } else {
        final storedToken = await _authRepository.getAccessToken();
        if (storedToken != null) {
          await _authRepository.logout(storedToken);
        }
      }
  ...
  Future<void> _acceptFriendRequest(int friendshipId) async {
    try {
      ...
      if (authState is AuthSuccess) {
        ...
        final storedToken = await _authRepository.getAccessToken();
        if (storedToken == null) throw '로그인 정보가 없습니다.';
        accessToken = storedToken;
      }

      await _friendRepository.acceptFriendRequest(accessToken, friendshipId);
      await _loadData();
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
  ...
  Future<void> _rejectFriendRequest(int friendshipId) async {
    try {
      ...
      await _friendRepository.rejectFriendRequest(accessToken, friendshipId);
      await _loadData();
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
  ...
  Future<void> _showCreateMapDialog() async {
    ...
    if (result == true && mounted) {
      try {
        ...
        final newMapId = await _homeRepository.createMap(
          accessToken,
        ...
        if (mounted) {
          Navigator.push(
            context,
            MaterialPageRoute(
              builder: (_) => MapDetailScreen(mapId: newMapId),
            ),
          );
        }
      } catch (e) {
      ...
      child: ListView(
        ...
        children: [
          ...
          if (_mapList.isEmpty)
            ...
            ..._mapList.map((map) => _buildMapListItem(map)),
        ],
      ),
    );
  }

  Widget _buildMapListItem(MapCardModel map) {
    return Container(
      ...
      child: ListTile(
        ...
        onTap: () {
          Navigator.push(
            context,
            MaterialPageRoute(
              builder: (_) => MapDetailScreen(mapId: map.mapId),
            ),
          );
        },
      ),
